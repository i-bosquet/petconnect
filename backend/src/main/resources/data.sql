...
-- =============================================
-- After manually inserting IDs that use the sequence,
-- tell the sequence generator what the next value should be
-- to avoid conflicts when JPA inserts new entities later.
-- Set it to the next value AFTER the highest ID we manually inserted (which was 10).
-- The function setval(sequence_name, next_value, is_called)
-- is_called=true means the next call to nextval will return next_value + increment
-- is_called=false means the next call to nextval will return next_value itself. We usually want true.
-- However, simpler is setting it to the max value used. The next nextval call will return max + 1.
SELECT setval('entity_id_sequence', 10, true); -- Sets the sequence's next value to be 11