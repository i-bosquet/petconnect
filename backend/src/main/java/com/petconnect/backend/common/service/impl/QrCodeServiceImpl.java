...
public class QrCodeServiceImpl implements QrCodeService {
    ...
    private byte[] convertJsonToCbor(String jsonPayload) {
        ...
        try {
            Map<String, Object> payloadMap = jsonMapper.readValue(jsonPayload, new TypeReference<>() {
            });
    ...
    private byte[] createCoseStructure(Certificate certificate, byte[] payloadCbor) throws CoseException {
        ...
        byte[] vetSignatureBytes = Base64.getDecoder().decode(certificate.getVetSignature());
        byte[] clinicSignatureBytes = Base64.getDecoder().decode(certificate.getClinicSignature());
    ...
    private CBORObject buildCoseSignature(byte[] signatureBytes, AlgorithmID algorithmID)  {
        try {
            ...
            protectedMap.Add(HeaderKeys.Algorithm.AsCBOR(), algorithmID.AsCBOR());
